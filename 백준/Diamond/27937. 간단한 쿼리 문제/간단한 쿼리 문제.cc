    ...
    for (query q : queries) {
        if (q.sq != sqq) {
            ...
            for (int i=q.left-1;i<q.right;i++) {
                temp += cnt.query(1, 1, arr[i], 1, sts) * arr[i];
                temp -= value.query(1, 1, arr[i], 1, sts);
                temp -= cnt.query(1, arr[i], N, 1, sts) * arr[i];
                temp += value.query(1, arr[i], N, 1, sts);

                cnt.update(arr[i], 1);
                value.update(arr[i], arr[i]);
            }
        } else {
