...
class Seg {
  ...
  public int queryMIN(int node, int s, int e, int left, int right) {
    if (orLazy[node] != 0 || andLazy[node] != FULL) propagate(node);
    ...
    return Math.min(queryMIN(node << 1, s, e, left, mid)
        , queryMIN(node << 1 | 1, s, e, mid+1, right));
  }
  
  private void propagate (int node) {
    tree[node].min &= andLazy[node];
    tree[node].min |= orLazy[node];
  ...
  public static void main(String[] args) throws IOException {
    ...
    while(Q-->0) {
      ...
      if (M == '&') {
        ...
        bw.write(tree.queryMIN(1, left, right, 1, tree.start)+"\n");
      }
    }
